...
setwd("/Users/jamessantangelo/Documents/Academia/Master's/SEC - Simulating evolutionary clines/Data, R code and figures/Datasets/Drift/Nvary")
datNvary<-fread("20160906_SEC_Drift_Nvary.results.csv",header = T)

#Dataset to correborrate previous results using migration script
setwd("/Users/jamessantangelo/Documents/Academia/Doctorate (PhD)/Projects/SEC - Simulating.evolutionary.clines/SEC_Data/Drift.Migration")
datNvary.MigTest <- fread("20170223_SEC_Drift.Migration.Test.N.merged.csv", header = T)
...
setwd("/Users/jamessantangelo/Documents/Academia/Master's/SEC - Simulating evolutionary clines/Data, R code and figures/Datasets/Drift/pApBvary")
...
###Itereates over data set and for every simulation of every variation of the population
##size parameter, runs a regression of phenotype frequency against generation number.
#Creates table out of results (i.e. intercepts and slopes)
list_reg <- dlply(datNvary, .(N, sim), function(datNvary) 
{lm(Phen~step,data=datNvary)})
Slopes = ldply(list_reg, coef)

#Summary dataset showing mean slope by population size and proportion of positive slopes
SlopeSum <- ddply(Slopes,.(N),summarise,mean = mean(step),prop = (sum(step > 0)/length(step)))

#Plot mean slope by population size
plotMeanSlope.x.N<-ggplot(SlopeSum, aes(x = factor(N), y = mean)) + 
  ylab("Mean slope")+xlab("Population size")+geom_bar(stat="identity", color = "black",fill="grey")
plotMeanSlope.x.N+ng1.45

#Plot proportion of positive slopes by population size
plotPropSlope.x.N<-ggplot(SlopeSum, aes(x = factor(N), y = prop)) + 
  ylab("Proportion positive slopes")+xlab("Population size")+geom_bar(stat="identity", color = "black",fill="grey")
plotPropSlope.x.N+ng1.45
...
###Itereates over data set and for every simulation of every variation of initial 
##allele frequencies, runs a regression of phenotype frequency against generation number.
#Creates table out of results (i.e. intercepts and slopes)
list_reg_pApB <- dlply(datpApBVary, .(pA.pB, sim), function(datpApBVary) 
{lm(Phen~step,data=datpApBVary)})
Slopes_pApB = ldply(list_reg_pApB, coef)

#Summary dataset showing mean slope by population size and proportion of positive slopes
SlopeSum_pApB <- ddply(Slopes_pApB,.(pA.pB),summarise,mean = mean(step),prop = (sum(step > 0)/length(step)))

SlopeSum_pApB <- tidyr::separate(SlopeSum_pApB, pA.pB, into = c("pA","pB"), ,remove = FALSE, sep = ";")
SlopeSum_pApB$Diff <- as.numeric(SlopeSum_pApB$pB) - as.numeric(SlopeSum_pApB$pA)
SlopeSum_pApB <-  within(SlopeSum_pApB, {
  Col = ifelse(Diff == 0, 1, 2)
})


SlopeSum_pApB$pA.pB <- factor(SlopeSum_pApB$pA.pB, 
...
plotMeanSlope.x.pApB<-ggplot(SlopeSum_pApB, aes(x = factor(pA.pB), y = mean)) + 
  ylab("Mean slope")+xlab("Initial dominant allele frequencies")+
  geom_bar(stat="identity", color = "black", aes(fill=factor(Col)))+
  coord_cartesian(ylim=c(0,0.003))+
  scale_y_continuous(breaks=seq(from = 0, to = 0.003, by = 0.001))+
  scale_fill_manual(values=c("grey","red"),
                    name="Staring alleles",
                    breaks=c("1", "2"),
                    labels=c("Identical", "Different"))
plotMeanSlope.x.pApB+ng1.45

plotPropPosSlope.x.pApB<-ggplot(SlopeSum_pApB, aes(x = factor(pA.pB), y = prop)) + 
  ylab("Proportion positive slopes")+xlab("Initial dominant allele frequencies")+
  geom_bar(stat="identity", color = "black",aes(fill=factor(Col)))+
  scale_fill_manual(values=c("grey","red"),
                  name="Staring alleles",
                  breaks=c("1", "2"),
                  labels=c("Identical", "Different"))
plotPropPosSlope.x.pApB+ng1.45

######################################################
#### VARY POPULATION SIZE — MIGRATION TEST SCRIPT ####
######################################################

##Summary dataset showing mean phenotype frequency (recessive) and mean allele frequencies
#by generation number and population size. Averaged across number of simulations
NvarySum.MigTest<-ddply(datNvary.MigTest,.(Pop_size,Generation),summarise, Phen = mean(Acyan), pA = mean(pA), pB = mean(pB))

##Plot of the frequency of recessive phenotype by the number of generations with 
#different lines for different population sizes. Averaged across 1000 simulations
plotPhen.StepNvary.MigTest<-ggplot(NvarySum.MigTest, aes(x = Generation, y = Phen ,group = Pop_size)) + 
  ylab("Frequency of recessive
       phenotype")+xlab("Generation number")+geom_line(size=0.5, aes(color = factor(Pop_size)))+
  labs(colour = "Pop. size")+
  coord_cartesian(ylim = c(0.42,0.78))+
  scale_y_continuous(breaks=seq(from = 0.42, to = 0.78, by = 0.04))
plotPhen.StepNvary.MigTest+ng1

#Dataset containing the first row from each simulation where the recessive phenotype is fixed
Nvary_Phen.fixU.MigTest <- datNvary.MigTest[datNvary.MigTest$Acyan == 1.0,]
Nvary_Phen.fixU.MigTest <- Nvary_Phen.fixU.MigTest[,unique(Nvary_Phen.fixU.MigTest, by = c('Pop_size','Sim'))]

#Dataset containing the first row from each simulation where the dominant phenotype is fixed
Nvary_Phen.fixD.MigTest <- datNvary.MigTest[datNvary.MigTest$Acyan == 0.0,]
Nvary_Phen.fixD.MigTest <- Nvary_Phen.fixD.MigTest[,unique(Nvary_Phen.fixD.MigTest, by = c('Pop_size','Sim'))]

#Datasets for number of simulations resulting in fixation of either phenotpype
Nvary_fixU.MigTest <- ddply(Nvary_Phen.fixU.MigTest,.(Pop_size),summarise,prop=(length(Pop_size)/1000))
Nvary_fixU.MigTest <- rbind(Nvary_fixU.MigTest,c(500,0.000))
Nvary_fixD.MigTest <- ddply(Nvary_Phen.fixD.MigTest,.(Pop_size),summarise,prop=(length(Pop_size)/1000))
Nvary_fixD.MigTest <- rbind(Nvary_fixD.MigTest,c(200,0.000),c(300,0.000),c(400,0.000),c(500,0.000))
Nvary_fix.MigTest <- merge(Nvary_fixU.MigTest,Nvary_fixD.MigTest,by.x ="Pop_size",by.y ="Pop_size")
setnames(Nvary_fix.MigTest,old=c("prop.x","prop.y"),new=c("FixU","FixD"))
Nvary_fix.MigTest$FixTot <- Nvary_fix.MigTest$FixU + Nvary_fix.MigTest$FixD

##Dataset showing mean time to fixation of recessive phenotype for each population size
#Also mean frequency of both alleles at phenotype fixation
Nvary_fixGen.MigTest <- ddply(Nvary_Phen.fixU.MigTest,.(Pop_size),summarise,meanGen = mean(Generation),
                      mean.pA = mean(pA),mean.pB = mean(pB))

#Plot showing mean number of generation to fixation of recessive phenotype by populations size
plotGen.N.Fix.MigTest<-ggplot(Nvary_fixGen.MigTest, aes(x = factor(Pop_size), y = meanGen)) + 
  ylab("Mean generations
to fixation")+xlab("Population size")+geom_bar(stat="identity", color = "black",fill="grey")+
  coord_cartesian(ylim = c(0,50))+
  scale_y_continuous(breaks=seq(from = 0, to = 50, by = 5))
plotGen.N.Fix.MigTest+ng1.45

###Itereates over data set and for every simulation of every variation of the population
##size parameter, runs a regression of phenotype frequency against generation number.
#Creates table out of results (i.e. intercepts and slopes)
list_reg.MigTest <- dlply(datNvary.MigTest, .(Pop_size, Sim), function(datNvary.MigTest) 
{lm(Acyan~Generation,data=datNvary.MigTest)})
Slopes = ldply(list_reg.MigTest, coef)

#Summary dataset showing mean slope by population size and proportion of positive slopes
SlopeSum.MigTest <- ddply(Slopes,.(Pop_size),summarise,mean = mean(Generation),prop = (sum(Generation > 0)/length(Generation)))

#Plot mean slope by population size
plotMeanSlope.x.N.MigTest<-ggplot(SlopeSum.MigTest, aes(x = factor(Pop_size), y = mean)) + 
  ylab("Mean slope")+xlab("Population size")+geom_bar(stat="identity", color = "black",fill="grey")
plotMeanSlope.x.N.MigTest+ng1.45

#Plot proportion of positive slopes by population size
plotPropSlope.x.N.MigTest<-ggplot(SlopeSum.MigTest, aes(x = factor(Pop_size), y = prop)) + 
  ylab("Proportion positive slopes")+xlab("Population size")+geom_bar(stat="identity", color = "black",fill="grey")
plotPropSlope.x.N.MigTest+ng1.45
